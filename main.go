...
func main() {
	//var token = "de7ef085dddb484e6a83d53e19c254bb5eaf7b8d8bdb5bec2967199654d72dee"
	...
	//	util.NewMailSender(config.NodeName, "ailabcircuitbreaker.dgu@gmail.com", "ZqlTp3985!", []string {"4whomtbts@gmail.com"}),
	//	10, config.NodeName, config.Exporters)

	if config.Type == "master" {
		m := &master.Master{
			MailSender: util.NewMailSender(config.NodeName, config.Email, config.EmailPassword, config.EmailReceivers),
		}
		m.Start(config.Nodes, 1500, 1500)
